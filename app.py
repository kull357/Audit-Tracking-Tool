...
# import os
# import re  # <-- add at the top of your file if not already there
...
#         "audit_assigned_date", "audit_assigned_time",
#         "auditor_name", "audit_comments", "audit_status",
...
# #     # ✅ Format URLs: remove unwanted whitespace, ensure comma before each URL except the first
# #     # raw_urls = data.get("urls_list", "")
# #     # url_list = [url.strip() for url in raw_urls.replace("\n", " ").replace(",", " ").split() if url.strip()]
# #     # formatted_urls = ", ".join([url for url in url_list if url.lower().startswith("http")])

# #         raw_urls = data.get("urls_list", "")
# #     url_list = re.findall(r'https?://[^\s,]+', raw_urls)
# #     formatted_urls = ", ".join(url_list)

# #     new_row = [
# #         data.get("ticket_id"),
# #         data.get("ticket_name"),
# #         data.get("BLI"),
# #         data.get("request_assigned_date"),
# #         data.get("request_due_date"),
# #         data.get("developer_name"),
# #         formatted_urls,  # ✅ Cleaned and comma-separated URLs
# #         data.get("no_of_urls"),
# #         data.get("audit_type"),
# #         audit_date,
# #         audit_time,
...
#     # ✅ Step 1: Extract and normalize URLs
#     raw_urls = data.get("urls_list", "")
    
#     # Insert a space before every http/https occurrence (except the first)
...
#     # Join with proper comma separation
...
#             if any(not row[j] for j in range(11, 16)):
...
#         data["Row Number"] = row_number  # ✅ Add this line
#         return jsonify(data)
#     else:
#         return jsonify({"message": "Row not found"}), 404
...
#         # row[11].value = data.get("auditor_name")
